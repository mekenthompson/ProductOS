...
- **`examples/`** in this repo carries the same shapes filled in for Tempo, a fictional team-scheduling product: a [product spec](../examples/product-spec-example.md), a [Job Spec](../examples/job-spec-example.md), [Job Links](../examples/job-links-example.md), an [RFC](../examples/rfc-example.md), and a full [strategy example](../examples/strategy-example/README.md). Fictional, so it's safe to read end to end without knowing switchroom's business.
