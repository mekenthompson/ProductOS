...
Does the proposed approach pass each of your [product principles](product-principles.md)? Run the check questions for each principle against the proposed solution.
...
**> 2 weeks → Full Spec**
Write a full [RFC](../templates/rfc.md) with RICE score. Get senior product sign-off (SVP Product). Follow the full delivery process. (The RFC references the relevant [Job Spec](../templates/job-spec.md) — the durable outcome contract that outlives this initiative.)
...
### Vision Check
How does this move the headline metric from the [product vision](../product-vision.md)?
```
